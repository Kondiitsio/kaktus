<div x-data="{ open: false }">
    <x-secondary-button class="mt-3" @click="open = true">{{ __('Lisää tuote') }}</x-secondary-button>
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                    ...
                    <form novalidate wire:submit.prevent="create">
                        ...
                        <div class="flex">
                            {{-- <livewire:admin.media.media-library :category="'product'" slot="mediaLibrary" /> --}}
                            <livewire:admin.product.product-create-media-picker />
                            {{-- <livewire:admin.media.media-preview :mediaId="$product['media_id']" slot="mediaPreview" /> --}}
                            <livewire:admin.product.product-create-media-preview />
                        </div>
                        <x-input-error class="mt-2" :messages="$errors->get('product.media')" />
                        </div>
                        </div>
                        <div class="gap-2 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <x-primary2-button wire:click.prevent="create" class="mt-3">Tallenna</x-primary2-button>
